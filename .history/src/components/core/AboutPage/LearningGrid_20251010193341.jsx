...
const LearningGrid = () => {
  ...
  return (
    <div>
        <div className='grid xl:grid-cols-3 grid-cols-1 grid-rows-2 w-[350px] xl:w-fit mx-auto mb-10 mt-16 py-10'>
            {LearningGridArray.map((card,index)=>(
              <div key={index} className={`
              ${index===0 && 'col-span-2 h-[294px]'}
              ${card.order % 2 !==0 ? 'bg-richblack-700 h-[294px]' : 'bg-richblack-800 h-[294px]' }
              ${card.order===3 && 'col-start-2'}`}>
                   {
                    card.order<0 ? (
                      <div className='w-[90%] flex flex-col gap-3 bg-richblack-900'>
                        <div className='w-[75%] text-4xl text-richblack-5 mb-4'>
                            {card.heading}
